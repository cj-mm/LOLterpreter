...
class Lexer(object):
    ...
    def __init__(self, program, on_error=None):
        """
        program   : the raw LOLCODE source string
        on_error  : optional callable(message) for reporting errors.
                    If None, errors are just printed to stdout.
        """
        self.program = program
        self.on_error = on_error or (lambda msg: print(msg))
        # flag if the program passed the lexical analysis, initialize to true
        self.check = 1
    ...
    def remove_comments(self, program, yarn_temp):
        temp = (
            "\n " + program
        )  # add new line and space to consider the occurence of a comment in the first line
        ...
        for i in range(len(prog_arr)):
            if bool(re.search("\sBTW", prog_arr[i])) or bool(
                re.search("\n\s*OBTW", prog_arr[i])
        ...
        for item in prog_arr:
            ...
            for i in range(len(temp)):
                if (
                    len(temp[i]) > 0 and temp[i][0] == '"' and temp[i][-1] == '"'
                ):  # yarn
    ...
    def classify_keywords(self, word):
        if word == "HAI" or word == "KTHXBYE":
            return "CODE_DELIMITER"
        # literal type
        elif word == "NUMBR" or word == "NUMBAR" or word == "YARN" or word == "TROOF":
            return "TYPE"
        # variable declaration
        elif word == "I HAS A":
            return "VARIABLE_DEC"
        # variable assignment
        elif word == "ITZ" or word == "R":
        ...
        elif word == "BOTH SAEM" or word == "DIFFRINT":
            return "COMPARISON_OP"
        # typecasting
        elif word == "MAEK" or word == "IS NOW A" or word == "R MAEK":
            return "TYPECAST"
        # printing
        elif word == "VISIBLE":
            return "OUTPUT_KEY"
        ...
        # conditional statements
            ...
            return "IF_THEN_DELIMITER"
        elif word == "YA RLY" or word == "NO WAI":
            return "IF_ELSE"
        elif word == "WTF?":
            return "SWITCH_DELIMITER"
        elif word == "OMG" or word == "OMGWTF":
            return "SWITCH_CASE"
        elif word == "OIC":
            return "COND_END"
        # loops
        elif word == "IM IN YR" or word == "IM OUTTA YR":
            return "LOOP"
        # function
        elif word == "HOW DUZ I" or word == "IF YOU SAY SO":
            return "FUNCTION"
        else:
            return "KEYWORD"
    ...
    def tokenize(self):
        ...
        for line in program:
            ...
            line = (
                line.split()
            )  # we can now split using space since keywords with spaces are already evaluated
            ...
            for word in line:
                if word == '"this_is_keyword2"':
                    temp = self.classify_keywords(
                        keywords2_temp[keyword2_num]
                    )  # look for the classification
                    tokens.append(
                        [temp, keywords2_temp[keyword2_num]]
                    )  # update the token
                    keyword2_num += 1
