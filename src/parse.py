...
class Parser(object):
    ...
    def __init__(self, tokens, on_error=None):
    ...
    def loops(self):
        if self.tokens[self.i][1] == "IM IN YR":
            ...
            if self.identifier():
                if (
                    self.tokens[self.i][1] == "UPPIN"
                    or self.tokens[self.i][1] == "NERFIN"
                ):
                    self.i += 1
                    if self.yr_key():
                        if self.identifier():
                            if (
                                self.tokens[self.i][1] == "TIL"
                                or self.tokens[self.i][1] == "WILE"
                            ):
                                self.i += 1
                                if self.expr():
                                    if self.loops_next():
